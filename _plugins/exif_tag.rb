...
module Jekyll
  class ExifTag < Liquid::Tag
    ...
    def render(context)
      exif = EXIFR::PNG::new(@image_file)
      <<-HTML
 #{exif.xresolution.to_i}  F#{sprintf "%.1f", exif.f_number.to_f} #{exif.model} ISO #{exif.iso_speed_ratings} #{exif.exposure_time.to_i}  #{exif.date_time_original}
      HTML
    end
  end
end
